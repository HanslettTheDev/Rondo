...
from rondo.custom_decorators import role_required
...
from rondo.superadmin.utils import (
    check_permission,
    delete_previous_permissions,
...
def dashboard():
    ...
    users = [
        user[0]
        for user in users
        if (user[0].role.name == "user")
        and ("pending" in get_permission_strings(user[0].permissions))
    ]

    return render_template("superadmin/main.html", title="Dashboard", users=users)


@superadmin.route("/approve_user/<int:user_id>")
@login_required
@role_required("superadmin")
def approve_user(user_id):
    user = db.session.execute(db.select(Users).filter_by(id=user_id)).fetchone()
    new_permsisions = [perm for perm in user[0].permissions if perm.name != "pending"]

    user[0].permissions = new_permsisions
...
def inventory():
    ...
    if request.method == "POST":
        ...
        laptop_entry = LaptopTable(
            ...
            brand_model_name=model_name,
            brand_specifications=specs,
...
def user_management():
    ...
    if request.method == "POST":
        ...
        new_permsisions = (
            request.form["custom-permissions"].strip().split(" ")
            if request.form["is-default"].lower() == "no"
            else DEFAULT_ROLE_PERMISSIONS[new_role]
        )
        user = db.session.execute(
            db.select(Users).filter_by(username=users_account)
        ).first()
    ...
    return render_template(
        "superadmin/user_management.html",
        title="User Management",
        users=users,
        roles=roles,
        permissions=permsisions,
    )
